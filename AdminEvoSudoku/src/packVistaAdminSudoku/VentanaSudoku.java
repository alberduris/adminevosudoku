...
import packAdminSudoku.GestionSudoku;
...
public class VentanaSudoku extends JFrame implements Observer{
	 ...
	 * @throws IOException 
	 */
	public VentanaSudoku() throws LineUnavailableException, IOException, UnsupportedAudioFileException {
		GestionSudoku gS = GestionSudoku.getGestionSudoku();
		gS.reiniciar();
		gS.addObserver(this);
	...
	private String formaTiempo(){
		int minutos = gS.obtenerTiempo()/60;
		int segundos = gS.obtenerTiempo()-minutos*60;
		String min;
		String seg;
		if(minutos < 10){
	...
	private void crearCajas(){
		...
		for(int i = 0; i < MAX; i++){
			for (int j = 0; j < MAX; j++){
				num = String.valueOf(gS.obtenerNum(i, j));
				//SOLUCION COMPLETA
		...
		keyListener = new KeyListener(){
			...
			public void keyReleased(KeyEvent e) {
				...
				if(Character.isDigit(num)&& num != '0'){
					...
					if(activado[0]>=0){
						gS.anadirNumero(activado[0], activado[1], num);
					}
				}
	...
	private void todosLosNumeros(char pNum){
		boolean[][] casillaNum = gS.todosLosNumeros(pNum);
	...
	private JLabel crearJLabel(final int pI,final int pJ, String pNum){
		...
		MouseListener mouseListener1 = new MouseListener(){
			...
			public void mouseReleased(MouseEvent e) {
				...
				todosLosNumeros(gS.obtenerNum(pI, pJ));
				j.updateUI();					
			}
			@Override
	...
	private void modificar(int pI, int pJ){
		String texto="";
		if(gS.obtenerListaNotas(pI, pJ) != null){
			texto = "<html>";
			for(int k=0; k<MAX; k++){
				if(Character.isDigit(gS.obtenerListaNotas(pI, pJ)[k])){
					texto += gS.obtenerListaNotas(pI, pJ)[k];
				}else{
					texto += "&nbsp;";
				}
				if(k!=2 && k!= 5 && k!= 8){
			...
			if(listBotones[k] == null){
				...
				listBotones[k].addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent arg0) {
						...
						if(activado[0]>=0){
							gS.anadirNumero(activado[0], activado[1], numero.charAt(0));
						}
					}
	...
	private JButton getBtn1() {
		if(btn1 == null){
			...
			btn1.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					desactivar();
					boolean[][] casillasFallo =	gS.comprobarCorrecto();
				...
				for(int i = 0; i < MAX; i++){
					for(int j = 0; j<MAX;j++){
						if(activado[0]>=0 && cajas[i][j].getBackground()!=Color.blue){
							gS.borrarNumero(activado[0], activado[1]);
	...
	private void getDialogFinal(){
		...
		JLabel texto = new JLabel("Has conseguido: "+gS.puntuacionConPenalizacion()+" puntos.");
