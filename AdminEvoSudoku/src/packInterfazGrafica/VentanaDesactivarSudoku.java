...
public class VentanaDesactivarSudoku extends JDialog implements Observer {
	 ...
	 * @throws IOException 
	 */
	public VentanaDesactivarSudoku() throws LineUnavailableException, IOException, UnsupportedAudioFileException {
		gA = GestorAdministrador.getGestorAdministrador();
		cS = CatalogoSudoku.getCatalogoSudoku();
		gA.addObserver(this);
		if(cS.obtListaEstado(true).length == 0){
			...
			add(norte,"North");
			add(centro);
			if(cS.obtListaEstado(1, true).length != 0){
				crearNorte(cS.obtListaEstado(1, true)[0], 0);
			}else if(cS.obtListaEstado(2, true).length != 0){
				crearNorte(cS.obtListaEstado(2, true)[0], 1);
			}else if(cS.obtListaEstado(3, true).length != 0){
				crearNorte(cS.obtListaEstado(3, true)[0], 2);
			}else if(cS.obtListaEstado(4, true).length != 0){
				crearNorte(cS.obtListaEstado(4, true)[0], 3);
			}else if(cS.obtListaEstado(5, true).length != 0){
				crearNorte(cS.obtListaEstado(5, true)[0], 4);
			}
	...
	private void crearNorte(String pIdent, final int pDif){	
		...
		dif.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				String[] list = cS.obtListaEstado(dif.getSelectedIndex()+1, false);
				if(list.length == 0){
					dif.setSelectedItem(pDif);
				}else{
		...
		ident = new JComboBox<String>(cS.obtListaEstado(pDif+1, true));
		...
		if(btn1 == null){
			...
			btn1.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					int aux2 = Integer.parseInt(cS.obtListaEstado(dif.getSelectedIndex()+1, true)[0]);
					if(aux1 == aux2){
						num = Integer.parseInt((String) ident.getSelectedItem()) + 1;
					}else{
						num = Integer.parseInt((String) ident.getSelectedItem()) - 1;
					}
					gA.modificarEstadoSudoku(false);
					if(num == 0){
						if(cS.obtListaEstado(true).length == 0){
							crearDialogoNoExisten();
						}else{
							if(cS.obtListaEstado(1, true).length != 0){
								gA.introducirSudoku(Integer.parseInt(cS.obtListaEstado(1, true)[0]));
								crearNorte(cS.obtListaEstado(1, true)[0], 0);
							}else if(cS.obtListaEstado(2, true).length != 0){
								gA.introducirSudoku(Integer.parseInt(cS.obtListaEstado(2, true)[0]));
								crearNorte(cS.obtListaEstado(2, true)[0], 1);
							}else if(cS.obtListaEstado(3, true).length != 0){
								gA.introducirSudoku(Integer.parseInt(cS.obtListaEstado(3, true)[0]));
								crearNorte(cS.obtListaEstado(3, true)[0], 2);
							}else if(cS.obtListaEstado(4, true).length != 0){
								gA.introducirSudoku(Integer.parseInt(cS.obtListaEstado(4, true)[0]));
								crearNorte(cS.obtListaEstado(4, true)[0], 3);
							}else if(cS.obtListaEstado(5, true).length != 0){
								gA.introducirSudoku(Integer.parseInt(cS.obtListaEstado(5, true)[0]));
								crearNorte(cS.obtListaEstado(5, true)[0], 4);
							}
							centro.removeAll();
							crearPaneles();
	...
	private void crearDialogoFinal(){
		...
		JLabel txt = new JLabel("El Sudoku se ha desactivado correctamente");
	...
	private void crearDialogoNoExisten(){
		...
		JLabel txt = new JLabel("No existen Sudokus para desactivar");
		JButton boton = new JButton("Continuar");
